    ...
    <div className="min-w-80 w-max p-4 flex flex-col justify-between items-center rounded-md border border-[#4D5BCE] md:max-w-96 md:h-52">
      <div className="w-full mb-5">
        <div className="mb-3">
          ...
          <span className="text-xs text-[#43D9AD]">{company}</span>
        </div>
        <p className="text-sm text-[#607B96] text-justify">{description}</p>
      </div>
      <div className="w-full flex justify-between items-center">
        ...
        <span className="text-xs text-[#43D9AD]">
          {">"} {startDate && endDate ? formatPeriod(startDate, isCurrent, endDate) : 'Unperiodized'}
        </span>
      </div>
    </div>
  );
